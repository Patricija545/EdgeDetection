...
Mat Sobel ( Mat Gx, Mat Gy, Mat& input, int size )
{
    ...
    for ( int y = 0; y < input.rows; ++y )
    {
	    ...
	    //sqrt(ceil((Gx_sum*Gx_sum) + (Gy_sum*Gy_sum)));
...
int main( int argc, char** argv )
{
  ...
  //Primjena Gaussovog filtera
  Mat Gauss;
  GaussianBlur(input, Gauss, cv::Size(5,5), 1, 0, BORDER_DEFAULT );
  
  
  //Derivacijske maske Sobel detektora ruba
  int kernelsize = 3;
  ...
  Mat output = Sobel (kernelX, kernelY, input, kernelsize);
  ...
  //Spremanje nove fotografije
//  try {
// 	imwrite( "../../output/sobel_output.png", output );
//     }
...
//     
  ...
  displayImages( make_pair("input", input), make_pair ("sobel filter", output));
